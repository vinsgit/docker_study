...
isolate itself from other containers and its host machine and run the enviroment, which is a bit like a simplified version of the Linux environment
...
4. Next we are about to set the dockerfile
    ...
    * **Why we need to specify workdir**
      ...
      2.  I created a new Dockerfile in it . If we want to create a hello.txt with Shell in the app directory, 
      we just need to execute the rest of the commands except the first line in the Docker file
            ```ruby
             1	FROM alpine
             2	RUN mkdir /app
             3	RUN cd /app
      ...
      3. We execute `docker build` command
        ...
        Now  we see the error there ,witch says it cant find the hello.txt. So It turns out  docker image works not like Shell. When we use Shell, several lines of commands are in the same process, therefore ,
        the memory state which is modified  by the previous command will directly affect the latter command. But it works different with  docker image. When we build a docker image ,you can imagine that you are playing lego, the image will be built  layer by layer. The front layer is the fundation of the next layer. If the build of the font layer is completed, then it wont change anymore, and any change on the latter layer affect only on its own. 
        
        So, in this example , we did create the hello.txt ,its just we didnt create it at the layer, in which we wanted to execute `cat /app/hello.txt` （cat-> concatenate）
        
        Therefor if we wanna  specify a  working direcotry to every layer, we should use WORKDIR command 
        &emsp;

    * **Why i copy Gemfile and Gemfile.lock seperatly instead of copying all the files at the front**
     
      Since we know image is built layer by layer , So let's assume that  i copied all the files with 
      this command `COPY . /app ` in front of ` RUN bundle install` . Then im ready to build my image. 
      Since  some codes  were changed by me , the docker will start to execute all remaining commands  
      from the `COPY . /app ` on the Dockerfile. But according to my original configerations,
      if i didnt modify Gemfile and Gemfile.lock , then RUN bundle install wouldn't be executed.
      The commands in the dockerfile would start to be executed  from line 20.
...
5. The settings in my docker-compose.yml
    ...
    Before the end of the Rails application's container launch , the command：rake db:migrate (set by the Dockerfile CMD) need to be ran first, which means that the Container of the Rails App can be started only after the Container of the database is started, so you can see the following setting in` docker-compose.yml`
    ```ruby
    app:
      depends_on:
         - db
...
7.  To Check if the containers work fine 
    * In the meantime, we can check if we have successfully run both containers by commanding: `docker ps`.
      ![6](https://1.bp.blogspot.com/-tsfUPpKrJZk/Xpa-TOfUwwI/AAAAAAAAAEE/oJ1O9nNXOiEI_63t4C8Q5VPs_KDq3X6EwCLcBGAsYHQ/s1600/6.jpg)
    * Type  http://127.0.0.1:3000 on browser. We  can see the project running! 
      ![1](https://1.bp.blogspot.com/-rHZ7jWDXSRs/XpW4WGZ2boI/AAAAAAAAADc/2QWLR6bM4qYcCQtw5JxrdS3LtpwPkN54ACLcBGAsYHQ/s1600/1.png)

### Step 3: Puma with Nginx in docker 
> Goal: add Nginx settings to docker-compose.yml, let Nginx to handle traffic with Puma.

1. In order to make Puma work with Nginx, we need to bind Puma on the Unix domain socket rather than 
bind it on  TCP/IP network socke, so we need note off ENV.fetch("PORT") { 3000 } in the puma file
...
Now we are going to build a Nginx image base on the the offical Nginx image and customize our Nginx for handling request.
...
3. Modify docker-compose.yml to add Nginx service
    ...
    This time Nginx can work with Puma! Since Nginx is set to port 80 , Wou can directly type 127.0.0.1 into our browser to see our website. , there is no need to add :3000 as the port number
    ![after nginx image is buid up](https://1.bp.blogspot.com/-oQA9wcMB8pk/XpcRS0uDImI/AAAAAAAAAEQ/dFQ6FKWkY90IuYv7mcaU1ZCvD14Uq8XOACLcBGAsYHQ/s1600/7.png)
   
    Then try to use `docker ps`  to see the container we started.
    ![after nginx image is buid up](https://1.bp.blogspot.com/-gEamn2-Eyf4/XpcRTCi7RoI/AAAAAAAAAEU/9DN_IaJF3xQiYwwHiR_rAOGctEehoCQZwCLcBGAsYHQ/s1600/1586958267%25281%2529.jpg)